 <div class="status-main-post space-x-2">
        ...
        <div class="profile-section-post">
            ...
            <div class="post-header-post flex items-center"> 
                @php
                $groupedFaculties = \App\Models\User::facultyOptions();
                $flatFaculties = collect($groupedFaculties)->flatMap(fn($group) => $group)->toArray();
            
                $userFaculty = Auth::user()->faculty ?? null;
            
                $facultyCode = $post->faculty ?? null;
                $badgeLabel = $facultyCode && isset($flatFaculties[$facultyCode])
                                ? $flatFaculties[$facultyCode]
                                : '';
            
                $badgeUrl = $facultyCode
                            ? route('faculty.index', ['faculty' => $facultyCode])
                            : null;
            
                $sameFaculty = $facultyCode && $userFaculty === $facultyCode;
            ...
            @endphp
                ...
                <div class="album-detail flex-1 ml-6">
                  ...
                  <div class="flex items-center space-x-2">
                    ...
                    <small class="inline-flex items-center space-x-1 text-sm text-gray-400">
                      <span>from</span>
                      <a href="{{ $badgeUrl }}"
                         class="text-blue-400 hover:underline"
                         onclick="return handleFacultyAccess({{ $sameFaculty ? 'true' : 'false' }});">
                        {{ $badgeLabel }}
                      </a>
                    </small>
                  </div>
              ...
              {{-- JavaScript: handle faculty link access --}}
              <script>
                function handleFacultyAccess(canAccess) {
                  if (!canAccess) {
                    alert('You are not from this faculty. To join, please update your profile.');
                    return false;
                  }
                  return true;
                }
