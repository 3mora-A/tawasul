...
<div class="album box" id="post-{{ $post->id }}">
              
                      
                        
    @include('partials.status-main')   
  ...
  {{ $post->content }}
                    ...
                    <div class="album-actions">
                        ...
                        class="album-action collapsed like-button {{ $post->likedBy(auth()->user()) ? 'liked' : '' }}"
                        data-post-id="{{ $post->id }}">
                            <svg stroke="currentColor" stroke-width="2"
                                fill="{{ $post->likedBy(auth()->user()) ? 'red' : 'none' }}"
                    ...
                    <div class="actions">
                        ...
                        <button class="comment-btn" onclick="submitComment({{ $post->id }})">Comment</button>
                    </div>
                    </div>

        
                    <!-- ✅ Only ONE comments container per post -->
                <div class="comments-wrapper">

                    @foreach ($post->comments as $comment)
                    <div class="comment-box">
                        <a href="{{ Auth::id() === $comment->user->id ? route('profile-mine') : route('profile.public', $comment->user->id) }}">
                            <img src="{{ $comment->user->profile_picture ? asset('storage/' . $comment->user->profile_picture) : asset('images/default_profile.png') }}" class="comments-profile-img" />
                        </a>
